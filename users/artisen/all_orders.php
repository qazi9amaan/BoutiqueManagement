...
<?php
    ...
    $pagelimit = 15;
    $search_str_all	= filter_input(INPUT_GET, 'search_str_all');
    ...
    $order_dir = 'Desc';
    $db = getDbInstance();
    $select = array('order_id', 'ordered_at', 'delivery_date', 'order_price', 'advance_money','finishing_date');
    $db->orderBy($order_by, $order_dir);
    $db->pageLimit = $pagelimit;
    if ($search_str_all) {
      $db->where('order_id', $search_str_all);
  }
    $rows = $db->arraybuilder()->paginate('completed_orders', $page, $select);
...
<table class="table">
  <caption class="">
      <?php echo paginationLinks($page, $total_pages, 'account.php'); ?>
  </caption>
