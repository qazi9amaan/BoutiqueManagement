...
<?php 
    ...
    {   
            ...
            $measurement_id = filter_input(INPUT_POST, 'measurement_id');

            $data = filter_input_array(INPUT_POST);
            $db = getDbInstance();
            
            $measurement_bol = @$data['measurements_changed'];
            ...
            if($measurement_bol){
                ...
                // UPDATING DEFAULT MEASUREMENT
                $data_cust['measurement_id'] =$measurement_id;
            ...
            $cust_bol = $db->where('cust_id',$cust_id)->update('customer', $data_cust);
            if (!$cust_bol)
            {   
                echo 'Insert failed: ' . $db->getLastError();
                exit();
